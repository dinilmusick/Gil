...
// src/libraries/RateLimiterConcepts/RateLimiterConcepts.js
var throttle = (key, intervalMs, state) => {
  const now = Date.now();
...
// src/entities/GithubManager/functionalities/StartWatchedClone/helper_functions/doWatch.ts
var runWatchLoop = async (jobKey, system, state) => {
  ...
  try {
    ...
    const allowed = FUNCTIONS.throttle(rateLimitKey, job.intervalMs || 3e4, state);
    if (allowed) {
      setTimeout(() => runWatchLoop(jobKey, system, state), job.intervalMs || 3e4);
    }
  } catch (e) {
    setTimeout(() => runWatchLoop(jobKey, system, state), job.intervalMs || 3e4);
  }
};
var doWatch = async (input, state, system) => {
  ...
  state.watchJobs[jobKey] = { active: true, alias: input.alias, owner: input.owner, repo: input.repo, url: input.url, intervalMs: input.intervalMs || 3e4 };
...
// src/entities/GithubManager/functionalities/StartWatchedClone/main_function/logic.ts
var schema = {
  input: external_exports.object({
    ...
    intervalMs: external_exports.number().optional()
  }),
  output: external_exports.object({
    status: external_exports.string(),
    jobKey: external_exports.string().optional()
  })
};
var main = async (input, { state, entity: system }) => {
  return await doWatch(input, state, system);
};

// src/entities/GithubManager/functionalities/StartWatchedClone/state_data/state.ts
var initialState = {};

// src/entities/GithubManager/functionalities/StartWatchedClone/StartWatchedClone.ts
var StartWatchedCloneAtom = {
...
// src/entities/GithubManager/functionalities/RepairRepo/helper_functions/doRepair.ts
import fs from "fs/promises";
import path from "path";

// src/libraries/GitHubConcepts/GitHubConcepts.js
var getRemoteRepoInfo = async (owner, repo, pat) => {
  if (pat === "mock-token")
    return { name: repo };
  const url = `https://api.github.com/repos/${owner}/${repo}`;
  return await callApi(url, pat);
};
var listPullRequests = async (owner, repo, pat) => {
  if (pat === "mock-token")
    return [{ number: 1 }];
  const url = `https://api.github.com/repos/${owner}/${repo}/pulls?state=all`;
  return await callApi(url, pat);
};
var createPullRequest = async (owner, repo, title, head, base, body, pat) => {
  if (pat === "mock-token")
    return { title };
...
var listIssues = async (owner, repo, pat) => {
  if (pat === "mock-token")
    return [{ number: 1 }];
  const url = `https://api.github.com/repos/${owner}/${repo}/issues?state=all`;
  return await callApi(url, pat);
};
var createIssue = async (owner, repo, title, body, pat) => {
  if (pat === "mock-token")
    return { title };
...
var getRepoTree = async (owner, repo, sha, pat) => {
  if (pat === "mock-token")
    return { tree: [] };
  const url = `https://api.github.com/repos/${owner}/${repo}/git/trees/${sha}?recursive=1`;
  return await callApi(url, pat);
};
var callApi = async (url, pat, options = {}) => {
  if (pat === "mock-token")
    return { mock: true };
  const headers = {
...
// src/entities/GithubManager/functionalities/RepairRepo/helper_functions/doRepair.ts
var doRepair = async (input, state, system) => {
  ...
  const repairFile = async (file) => {
    const data = await FUNCTIONS2.downloadFileContent(input.owner, input.repo, file.path, keyRes.value);
...
// src/entities/GithubManager/functionalities/RepairRepo/main_function/logic.ts
var schema2 = {
  input: external_exports.object({
    alias: external_exports.string(),
    owner: external_exports.string(),
    repo: external_exports.string(),
    missingFiles: external_exports.array(external_exports.any())
  }),
  output: external_exports.object({
    status: external_exports.string(),
    repairedCount: external_exports.number().optional(),
    error: external_exports.string().optional()
  })
};
var main2 = async (input, { state, entity: system }) => {
  return await doRepair(input, state, system);
};

// src/entities/GithubManager/functionalities/RepairRepo/state_data/state.ts
var initialState2 = {};

// src/entities/GithubManager/functionalities/RepairRepo/RepairRepo.ts
var RepairRepoAtom = {
  id: "RepairRepo",
...
// src/entities/GithubManager/functionalities/VerifyRepoIntegrity/helper_functions/verifyTree.ts
import fs2 from "fs/promises";
import path2 from "path";
var verifyTree = async (localRepoDir, tree) => {
  const missing = [];
  const check = async (n) => {
    if (n.type !== "blob") return;
    const p = path2.join(localRepoDir, n.path);
    const sz = await fs2.stat(p).then((s) => s.size).catch(() => -1);
...
// src/entities/GithubManager/functionalities/VerifyRepoIntegrity/helper_functions/doVerify.ts
var doVerify = async (input, state, system) => {
  const krng = system._ingressNetwork?.members?.get("krng")?.entity;
  if (!krng) return { status: "error", error: "Krng service not available" };
  const keyRes = await krng.call("RetrieveKey", { id: `github:pat:${input.alias}` });
  if (keyRes.status !== "success" || !keyRes.value) return { status: "error", error: "No token" };
  try {
    let sha = input.sha;
    if (!sha) {
      const info = await FUNCTIONS2.getRemoteRepoInfo(input.owner, input.repo, keyRes.value);
      sha = info.default_branch || "main";
    }
    const treeRes = await FUNCTIONS2.getRepoTree(input.owner, input.repo, sha, keyRes.value);
    const account = state.accounts[input.alias];
    const localRepoDir = `${account.workspace}/${input.repo}`;
...
// src/entities/GithubManager/functionalities/VerifyRepoIntegrity/main_function/logic.ts
var schema3 = {
  input: external_exports.object({
    alias: external_exports.string(),
    owner: external_exports.string(),
  ...
  output: external_exports.object({
    status: external_exports.string(),
    missingFiles: external_exports.array(external_exports.any()).optional(),
    totalFiles: external_exports.number().optional(),
    error: external_exports.string().optional()
  })
};
var main3 = async (input, { state, entity: system }) => {
  return await doVerify(input, state, system);
};

// src/entities/GithubManager/functionalities/VerifyRepoIntegrity/state_data/state.ts
var initialState3 = {};

// src/entities/GithubManager/functionalities/VerifyRepoIntegrity/VerifyRepoIntegrity.ts
var VerifyRepoIntegrityAtom = {
...
// src/entities/GithubManager/functionalities/CloneRepo/helper_functions/doClone.ts
import fs3 from "fs/promises";

// src/libraries/GitOpsConcepts/GitOpsConcepts.js
var cloneRepository = async (url, dest, pat) => {
  if (pat === "mock-token")
...
var runGitCommand = async (args, cwd) => {
...
// src/entities/GithubManager/functionalities/CloneRepo/helper_functions/doClone.ts
var doClone = async (input, state, system) => {
  const krng = system._ingressNetwork?.members?.get("krng")?.entity;
  if (!krng) return { status: "error", error: "Krng service not available" };
  const keyRes = await krng.call("RetrieveKey", { id: `github:pat:${input.alias}` });
  if (keyRes.status !== "success" || !keyRes.value) return { status: "error", error: "No token" };
  ...
  let exists = false;
  try {
    const files = await fs3.readdir(dest);
    exists = files.length > 0;
  } catch (e) {
  }
  if (!exists) {
    await FUNCTIONS3.cloneRepository(input.url, dest, keyRes.value);
  }
...
// src/entities/GithubManager/functionalities/CloneRepo/main_function/logic.ts
var schema4 = {
  input: external_exports.object({
    alias: external_exports.string(),
    owner: external_exports.string(),
...
var main4 = async (input, { state, entity: system }) => {
  return await doClone(input, state, system);
};

// src/entities/GithubManager/functionalities/CloneRepo/state_data/state.ts
var initialState4 = {};

// src/entities/GithubManager/functionalities/CloneRepo/CloneRepo.ts
var CloneRepoAtom = {
  id: "CloneRepo",
...
// src/entities/GithubManager/functionalities/GetRepoInfo/main_function/logic.ts
var schema5 = {
  input: external_exports.object({
    alias: external_exports.string(),
    owner: external_exports.string(),
  ...
  output: external_exports.object({
    status: external_exports.string(),
    info: external_exports.any().optional(),
    error: external_exports.string().optional()
  })
};
var main5 = async (input, { state, entity: system }) => {
  const krng = system._ingressNetwork?.members?.get("krng")?.entity;
  if (!krng) return { status: "error", error: "Krng service not available" };
  const keyRes = await krng.call("RetrieveKey", { id: `github:pat:${input.alias}` });
...
// src/entities/GithubManager/functionalities/GetRepoInfo/state_data/state.ts
var initialState5 = {};

// src/entities/GithubManager/functionalities/GetRepoInfo/GetRepoInfo.ts
var GetRepoInfoAtom = {
  id: "GetRepoInfo",
...
// src/entities/GithubManager/functionalities/ListRepos/main_function/logic.ts
var schema6 = {
  input: external_exports.object({
    alias: external_exports.string()
  }),
  output: external_exports.object({
    status: external_exports.string(),
    repos: external_exports.any().optional(),
    error: external_exports.string().optional()
  })
};
var main6 = async (input, { state, entity: system }) => {
  const krng = system._ingressNetwork?.members?.get("krng")?.entity;
  if (!krng) return { status: "error", error: "Krng service not available" };
  const keyRes = await krng.call("RetrieveKey", { id: `github:pat:${input.alias}` });
  if (keyRes.status !== "success" || !keyRes.value) {
  ...
  try {
    const repos = await FUNCTIONS2.listAccountRepos(keyRes.value);
...
// src/entities/GithubManager/functionalities/ListRepos/state_data/state.ts
var initialState6 = {};

// src/entities/GithubManager/functionalities/ListRepos/ListRepos.ts
var ListReposAtom = {
  id: "ListRepos",
...
// src/entities/GithubManager/functionalities/GetAccountInfo/main_function/logic.ts
var schema7 = {
  input: external_exports.object({
    alias: external_exports.string()
  }),
  output: external_exports.object({
    status: external_exports.string(),
    user: external_exports.any().optional(),
    error: external_exports.string().optional()
  })
};
var main7 = async (input, { state, entity: system }) => {
  const krng = system._ingressNetwork?.members?.get("krng")?.entity;
  if (!krng) return { status: "error", error: "Krng service not available" };
  const keyRes = await krng.call("RetrieveKey", { id: `github:pat:${input.alias}` });
  if (keyRes.status !== "success" || !keyRes.value) {
  ...
  try {
    const user = await FUNCTIONS2.getAccountUser(keyRes.value);
...
// src/entities/GithubManager/functionalities/GetAccountInfo/state_data/state.ts
var initialState7 = {};

// src/entities/GithubManager/functionalities/GetAccountInfo/GetAccountInfo.ts
var GetAccountInfoAtom = {
  id: "GetAccountInfo",
...
// src/entities/GithubManager/functionalities/ListAccounts/main_function/logic.ts
var schema8 = {
  input: external_exports.object({}),
  output: external_exports.object({
    status: external_exports.string(),
...
// src/entities/GithubManager/functionalities/ListAccounts/state_data/state.ts
var initialState8 = {};

// src/entities/GithubManager/functionalities/ListAccounts/ListAccounts.ts
var ListAccountsAtom = {
  id: "ListAccounts",
...
// src/entities/GithubManager/functionalities/RemoveAccount/main_function/logic.ts
var schema9 = {
  input: external_exports.object({
    alias: external_exports.string(),
    deleteFolder: external_exports.boolean().default(false)
  }),
  output: external_exports.object({
    status: external_exports.string(),
    alias: external_exports.string()
...
var main9 = async (input, { state, entity: system }) => {
  const krng = system._ingressNetwork?.members?.get("krng")?.entity;
  ...
  if (account && input.deleteFolder) {
    ...
    await fs4.rm(account.workspace, { recursive: true, force: true }).catch(() => {
    });
  }
...
// src/entities/GithubManager/functionalities/RemoveAccount/state_data/state.ts
var initialState9 = {};

// src/entities/GithubManager/functionalities/RemoveAccount/RemoveAccount.ts
var RemoveAccountAtom = {
  id: "RemoveAccount",
...
// src/entities/GithubManager/functionalities/AddAccount/main_function/logic.ts
var main10 = async (input, { state, entity: system }) => {
  const accountDir = `${input.workspace}/${input.alias}`;
  const fs4 = await import("fs/promises");
  await fs4.mkdir(accountDir, { recursive: true });
  const krng = system._ingressNetwork?.members?.get("krng")?.entity;
...
// src/entities/GithubManager/functionalities/AddAccount/state_data/state.ts
var initialState10 = {};

// src/entities/GithubManager/functionalities/AddAccount/AddAccount.ts
var AddAccountAtom = {
  id: "AddAccount",
...
// src/entities/GithubManager/GithubManager.ts
import { krnlEntityBase } from "krnlts";
var GithubManager = class extends krnlEntityBase {
  constructor(owner) {
...
var DocumentationManager = {
  layers: {
    LAYER_1_ENTRY: ["StartWatchedClone", "RepairRepo", "VerifyRepoIntegrity", "CloneRepo", "GetRepoInfo", "ListRepos", "GetAccountInfo", "ListAccounts", "RemoveAccount", "AddAccount"]
    // To be populated by DeaTS
  ...
  getDocForEndpoint(fid) {
    return this.endpoints[fid] || null;
  },
  getDocsForLayer(layerId) {
    const atomIds = this.layers[layerId] || [];
    return atomIds.map((id) => this.endpoints[id]).filter(Boolean);
  },
  getNextStepsDoc(fid) {
    const nextLayerIds = this.nextSteps[fid] || [];
    let docs = [];
...
// src/Gil.ts
var Gil = class extends krnlSystemEntityBase {
  constructor(commPort = 10091, host = "0.0.0.0") {
    super("Gil", null, commPort, host);
    this.addChild(new GithubManager(this));
    this.setDocumentationProvider(DocumentationManager);
  }
  addLocal(atom) {
    const originalApi = atom.api;
    const id = atom.id;
    const wrappedApi = async (input, context) => {
  ...
  async boot() {
    this.commComponent.defineRpc("call", async (payload) => {
    ...
    console.log("Gil System initialized.");
  }
};
if (import.meta.url === `file://${process.argv[1]}`) {
  const port = Number(process.env.KRNL_PORT || 10091);
  const system = new Gil(port);
  await system.boot();
}
export {
  Gil
